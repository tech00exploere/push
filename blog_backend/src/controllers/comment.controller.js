...
export const addComment = async (req, res) => {
  try {
    ...
    const comment = await Comment.create({ text: text.trim(), post: postId, author: req.user._id });
...
export const deleteComment = async (req, res) => {
  try {
    ...
    if (!isValidId(postId) || !isValidId(commentId)) return res.status(400).json({ message: "Invalid id" });
    ...
    const remainingComments = await Comment.find({ post: postId }).populate("author", "name").sort({ createdAt: -1 });
