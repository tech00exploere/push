...
export const deletePost = async (req, res) => {
  try {
    ...
    const post = await Post.findById(id);
    if (!post) return res.status(404).json({ message: "Post not found" });
    const postOwnerId = post.user?._id?.toString?.() || post.user?.toString?.() || post.author?.toString?.();
    if (!postOwnerId || postOwnerId !== req.user._id.toString()) return res.status(403).json({ message: "Not authorized" });
